...
## Paso 1. Hagan un fork (los dos caminos empiezan aquí)
...
## Paso 2. Elijan un camino

Hay dos formas de trabajar y **las dos son válidas**. Elijan una.

| | **A. Codespaces** | **B. Su computadora** |
|---|---|---|
| Instalar cosas | nada | git, uv, un editor |
| Dónde corre | en la nube, desde el navegador | en su máquina |
| Autenticación de git | ya viene lista | la configuran ustedes |
| Costo | horas gratis al mes por cuenta | gratis |
| Si su laptop es lenta | no importa | sí importa |

Si nunca han configurado un entorno de Python, **vayan por Codespaces**. Se ahorran el
error más común de esta tarea, que es pelearse con permisos de git.

---

## Camino A. GitHub Codespaces

Un **Codespace** es una computadora en la nube con VS Code en el navegador, ya
configurada con todo lo que necesitan. Este repo trae la configuración incluida: al
arrancar instala las dependencias y registra el kernel de Jupyter solo.

### A.1 Creen el Codespace

En **su fork** (verifiquen que la URL diga su usuario, no `nasaul`):

**Code** (botón verde) → pestaña **Codespaces** → **Create codespace on main**

La primera vez tarda unos minutos: está instalando el entorno. En la terminal van a ver
`Installing Python dependencies...` y `Registering Jupyter kernel...`. Esperen a que
termine.
...
mantiene su trabajo aislado y es lo que hace posible el Pull Request.

### A.3 Abran el notebook

`tareas/tarea_01_aproximacion_funcion.ipynb`. Arriba a la derecha, donde dice
**Select Kernel**, elijan **Python 3.13 (mineria-datos)**. Si eligen otro kernel les van
a faltar las librerías.

Sigan en el **paso 3** de abajo.

### A.4 Cuando terminen, apaguen el Codespace

GitHub les da un número de horas gratis al mes y un Codespace encendido las consume
aunque no lo estén usando. En <https://github.com/codespaces>, en los tres puntos de su
Codespace → **Stop codespace**. Sus archivos se quedan; al volver a abrirlo siguen ahí.

Detalles de las horas incluidas: <https://docs.github.com/es/billing/managing-billing-for-github-codespaces/about-billing-for-github-codespaces>.
Con [GitHub Education](https://education.github.com/) suelen ser más.

---

## Camino B. Su computadora
...
el `git push` del paso 4 va a fallar con un error de permisos — es la equivocación más
común de esta tarea.

### B.2 Instalen las dependencias
...
### B.3 Creen su rama y abran el notebook
...
`git add` selecciona qué cambios entran, `git commit` los guarda con un mensaje en el
repositorio local, y `git push` los sube a su fork en GitHub. Son tres pasos distintos y
vale la pena entender por qué.

Pueden hacer commit y push tantas veces como quieran: **subir cambios no gasta intentos**.

## Paso 5. Abran el Pull Request
...
| Síntoma | Causa |
|---|---|
| `git push` dice que no tienen permisos | Clonaron el repo del curso en lugar de su fork (camino B) |
| "No existe la carpeta vXXXXXXXX" | Su clave única está mal escrita en el notebook |
| `ModuleNotFoundError` en el notebook | Kernel equivocado: elijan **Python 3.13 (mineria-datos)** |
...
papeles diferentes según la persona. Puede explicarles teoría y listarles técnicas, pero
no puede saber cuál de sus gráficas de residuales tiene forma. Eso solo lo revela correr
el código.
